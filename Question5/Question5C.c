...
// Function to send a file as multiple DAT packets and receive their ACKs
void sendFile(int sockfd, struct sockaddr *server_addr, socklen_t server_addr_len, const char *filename) {
    ...
    while (1) {
        ...
        uint16_t block_number_network = htons(block_number);
        memcpy(dat_buffer + 2, &block_number_network, sizeof(uint16_t));
...
int main(int argc, char *argv[]) {
    ...
    sendFile(sockfd, res->ai_addr, res->ai_addrlen, filename);
